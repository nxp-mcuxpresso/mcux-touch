 ...
 *
 * The gesture feature is the most important part of the matrix control. The gestures can be divided to two groups:
 * control gestures and unlock gestures. 
 *
 * Control gesture
 * The control gestures are usually used to control the device, change the mode, configure a feature, 
 * user action. The unlock gestures stored by the user are typically used to unlock or make some
 ...
 *  - swipe and hold down
 *  - swipe and hold left
 *  - swipe and hold right
 *  - touch and hold one finger
 *  - touch and hold two fingers
 *  - zoom in
 *  - zoom out
 *
 * To recognize the cover gesture, each touchpad electrode must be touched. 
 * The double tap gesture enables to define the maximum time interval between taps. 
 *
 * The conditions to recognize the swipe gestures:
 *  - buffered gesture must be long at least three recognized positions horizontally or vertically (for one finger)
 ...
 *
 * The hold gestures can be recognized after the hold time interval (defined by the user). The touch or movement event 
 * must be the last event to start hold gesture recognition. The touch and hold gesture is defined by finger touch and 
 * maximal one position change during hold time. The two fingers touch and hold gesture is defined as one finger touch 
 * and hold gesture with condition at least one untouched electrodes between fingers. The hold gesture type depends on:
 *  - the release event will reset the hold time interval cause cancelation hold gesture
 *  - the touch and hold gesture requires keep finger or finger without moving during whole hold time
 *  - the swipe and hold gesture require the enough horizontal or vertical finger movement long through 2 electrodes and 
 *    then keeping finger for whole hold interval.
 *
 * The conditions to recognize the zoom in/out gestures:
...
enum nt_control_matrix_flags
{
    ...
    NT_MATRIX_HOLD_CONTROL_UP_GESTURE_FLAG    = 1 << NT_FLAGS_SPECIFIC_SHIFT(6), /*!< Control swipe up and hold detecting */
    NT_MATRIX_HOLD_CONTROL_DOWN_GESTURE_FLAG  = 1 << NT_FLAGS_SPECIFIC_SHIFT(7), /*!< Control swipe down and hold detecting */ 
    NT_MATRIX_HOLD_CONTROL_LEFT_GESTURE_FLAG  = 1 << NT_FLAGS_SPECIFIC_SHIFT(8), /*!< Control swipe left and hold detecting */ 
    NT_MATRIX_HOLD_CONTROL_RIGHT_GESTURE_FLAG = 1 << NT_FLAGS_SPECIFIC_SHIFT(9), /*!< Control swipe right and hold detecting */ 
};
...
struct nt_control_matrix_data
{
    ...
    uint32_t hold_next_time;       /*!< Time instant for one or two fingers hold gestures recognition */
    uint16_t touchpad_mask_horizontal; /*!< Touch-pad mask for the horizontal coordinate */
    uint16_t gesture_diff[NT_MATRIX_UNLOCK_GESTURES]; /*!< Calculated differences between the stored and 
                                                           buffered gestures */
