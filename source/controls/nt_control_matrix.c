...
uint16_t _nt_control_matrix_horiz_vertic_recognition(nt_coordinates64_xy *buffer, uint16_t finger)
{
    ...
    if ((buffer->x | buffer->y) >= 0x100U) /* too short gesture - shorter than 3 point changes will not be recognized */
    {
        temp64 = buffer->x;
...
static int32_t _nt_control_matrix_init(struct nt_control_data *control)
{
    ...
    { /* mask for mutual-cap touch pad */
        ...
        if (pins > (uint8_t)64) /* Maximum TSI mutual-cap channel restriction given by uint64 type status variable */
        {
            pins = 64U;
        }
    }
    ...
    { /* mask for self-cap touch pad */
        pins = rom->touchpad_size_horizontal + rom->touchpad_size_vertical;
        if (pins > 50U) /* Maximum TSI self-cap channel restriction */
        {
            pins = 50U;
        }
    }
    ...
    if ((bool)rom->hold_interval)  
    {    
        ram->hold_next_time = 0xffffffff; /* disable hold gesture because hold interval not set */ 
    }
    else
    {   /* Set hold time to avoid checking hold gesture on begining of process function */ 
        ram->hold_next_time = nt_system_get_time_counter() + rom->hold_interval;
    }  
    return (int32_t)NT_SUCCESS;
}
...
static int32_t _nt_control_matrix_process(struct nt_control_data *control)
{
    ...
    { /* touch pad electrode state for matrix processing */
        ...
        /*** TOUCH AND HOLD ONE OR TWO FINGERS AND SWIPE AND HOLD GESTURES DETECTION */
        if ((nt_system_get_time_counter() > ram->hold_next_time) && 
           (bool)_nt_control_get_flag(control, (int32_t)(NT_MATRIX_TOUCH_FLAG | NT_MATRIX_MOVEMENT_FLAG))) 
        {   ram->hold_next_time = 0xffffffff; 
            if ((bool)_nt_control_get_flag(control,
                (int32_t)(NT_MATRIX_HOLD_CONTROL_UP_GESTURE_FLAG   | NT_MATRIX_HOLD_CONTROL_DOWN_GESTURE_FLAG |
                          NT_MATRIX_HOLD_CONTROL_LEFT_GESTURE_FLAG | NT_MATRIX_HOLD_CONTROL_RIGHT_GESTURE_FLAG))) 
            {   /* hold gesture recognized so avoid same check in next function call */     
                _nt_control_set_flag(control, (int32_t)NT_MATRIX_GET_CONTROL_GESTURE_FLAG);
                
                /* Check what type swipe of swipe and hold gesture is going on and trigger specific callback */
                if (_nt_control_get_flag(control, (int32_t)NT_MATRIX_HOLD_CONTROL_UP_GESTURE_FLAG))
                {
                    _nt_control_matrix_invoke_callback(control, NT_MATRIX_GET_CONTROL_GESTURE,
                                                       &ram->release_position[0], (int32_t)NT_MATRIX_SWIPE_HOLD_UP);        
                    _nt_control_clear_flag(control, (int32_t)NT_MATRIX_HOLD_CONTROL_UP_GESTURE_FLAG);
                } else if (_nt_control_get_flag(control, (int32_t)NT_MATRIX_HOLD_CONTROL_DOWN_GESTURE_FLAG))
                {
                    _nt_control_matrix_invoke_callback(control, NT_MATRIX_GET_CONTROL_GESTURE,
                                                       &ram->release_position[0], (int32_t)NT_MATRIX_SWIPE_HOLD_DOWN);        
                    _nt_control_clear_flag(control, (int32_t)NT_MATRIX_HOLD_CONTROL_DOWN_GESTURE_FLAG);
                } else if (_nt_control_get_flag(control, (int32_t)NT_MATRIX_HOLD_CONTROL_LEFT_GESTURE_FLAG))
                {
                    _nt_control_matrix_invoke_callback(control, NT_MATRIX_GET_CONTROL_GESTURE,
                                                       &ram->release_position[0], (int32_t)NT_MATRIX_SWIPE_HOLD_LEFT);        
                    _nt_control_clear_flag(control, (int32_t)NT_MATRIX_HOLD_CONTROL_LEFT_GESTURE_FLAG);
                } else if (_nt_control_get_flag(control, (int32_t)NT_MATRIX_HOLD_CONTROL_RIGHT_GESTURE_FLAG))
                {
                    _nt_control_matrix_invoke_callback(control, NT_MATRIX_GET_CONTROL_GESTURE,
                                                       &ram->release_position[0], (int32_t)NT_MATRIX_SWIPE_HOLD_RIGHT);        
                    _nt_control_clear_flag(control, (int32_t)NT_MATRIX_HOLD_CONTROL_RIGHT_GESTURE_FLAG);
                } else
                {   /* Empty else section for previous if section */
                }
            }  /* Detecting type (one or two finges) touch and hold gestures */
            else if (ram->buffer_position <= 8U && 
                     !(bool)_nt_control_get_flag(control, (int32_t)NT_MATRIX_GET_CONTROL_GESTURE_FLAG)) 
            {    
                _nt_control_set_flag(control, (int32_t)NT_MATRIX_GET_CONTROL_GESTURE_FLAG);
                if(ram->touch_position[1].x + ram->touch_position[1].y == 0U)
                    _nt_control_matrix_invoke_callback(control, NT_MATRIX_GET_CONTROL_GESTURE,
                                                       &ram->touch_position[0], (int32_t)NT_MATRIX_1HOLD);
                else 
                    _nt_control_matrix_invoke_callback(control, NT_MATRIX_GET_CONTROL_GESTURE,
                                                       &ram->touch_position[0], (int32_t)NT_MATRIX_2HOLD);                 
            }   else
            {   /* Empty else section for previous if section */
            }
            
            /* Some hold gesture recognized, clear flag and go out */
        ...
        { /* Store last electrode states, x and y together. */
            ...
            if (state == (uint64_t)ram->touchpad_mask) /* all touch pad electrode are active */
            {
                ...
                _nt_control_clear_flag(control, (int32_t)NT_CONTROL_NEW_DATA_FLAG); /* data processed */
                return (int32_t)NT_SUCCESS; /* Cover gesture recognized, go out */
            }
            ...
            { /* x, y states for mutual-capacitance mode */
                ...
                { /* cut current state to line segments by right shifts AND operation */
                    line_segment = (uint16_t)((state >> (line * rom->touchpad_size_horizontal)) & 
                                               control->data.matrix->touchpad_mask_horizontal);
            ...
            { /* Detect position of one or two finger in x and y coordinates. */
                ...
                { /* CONTROL GESTURE DELAY */
                    ...
                    else /*** TOUCH OR MOVEMENT - DOUBLE TAB NOT RECOGNIZED */
                    {
                ...
                /*** TOUCH OR MOVEMENT - SWIPE AND HOLD RECOGNITION ***/
                if (ram->buffer_position >= 12U) /* Swipe gesture shall be long 3 position changes or more */
                {   /*** TOUCH OR MOVEMENT - SWIPE AND HOLD GESTURE RECOGNITION IN PROGRESS ***/
                    /* SWIPE CONTROL GESTURES RECOGNITION if one finger tracked in buffer or if hold gesture in progress */
                    ...
                    {   
                        ...
                        { /* Check if drawn track is enough horizontal or vertical for swipe gestures, skip after gesture
                            ...
                            { /* calculate the swipe direction */
                                ...
                                /* evaluate swipe for swipe and hold type according to direction */
                                if (dir_diff_y < dir_diff_x && dir_diff_y > -dir_diff_x && dir_diff_x > 0)
                                {   _nt_control_set_flag(control, (int32_t)NT_MATRIX_HOLD_CONTROL_UP_GESTURE_FLAG);
                                }
                                else if (dir_diff_y < -dir_diff_x && dir_diff_y > dir_diff_x && dir_diff_x < 0)
                                {   _nt_control_set_flag(control, (int32_t)NT_MATRIX_HOLD_CONTROL_DOWN_GESTURE_FLAG);
                                }
                                else if (dir_diff_x < dir_diff_y && dir_diff_x > -dir_diff_y && dir_diff_y > 0)
                                {   _nt_control_set_flag(control, (int32_t)NT_MATRIX_HOLD_CONTROL_LEFT_GESTURE_FLAG);
                                }
                                else if (dir_diff_x < -dir_diff_y && dir_diff_x > dir_diff_y && dir_diff_y < 0)
                                {   _nt_control_set_flag(control, (int32_t)NT_MATRIX_HOLD_CONTROL_RIGHT_GESTURE_FLAG);
                                }
                                else
                                {   /* avoid Misra issue */
                                }
                                ram->hold_next_time = nt_system_get_time_counter() + rom->hold_interval; /* Set hold time during movement */
                            }
            ...
            else if (!((bool)state_x || (bool)state_y))
            {
                ...
                /* No finger was recognized only one position with x or y values has been touched.*/
                if (ram->buffer_position == 0U)
                {
                ...
                {   _nt_control_set_flag(control, (int32_t)NT_MATRIX_RELEASE_FLAG);
                    _nt_control_clear_flag(control, (int32_t)(
                                NT_MATRIX_HOLD_CONTROL_UP_GESTURE_FLAG | NT_MATRIX_HOLD_CONTROL_DOWN_GESTURE_FLAG |
                                NT_MATRIX_HOLD_CONTROL_LEFT_GESTURE_FLAG | NT_MATRIX_HOLD_CONTROL_RIGHT_GESTURE_FLAG)); 
                }
