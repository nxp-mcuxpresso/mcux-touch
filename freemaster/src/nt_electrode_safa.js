...
NTElectrode_safa.prototype.OnLinkClicked = function()
{
    ...
    this.DefineSignalVariable(vname_measured_iir).then((res) => {
      if(res)
        var st = this.DefineFilteredSignalVariable(vname_predicted).then((res) => {
          if(res)
            var st = this.DefineBaselineVariable(vname_baseline).then((res) => {
              if(res)
                var st = this.DefineNoiseVariable(vname_noise).then((res) => {
                  if(res)
                    var st = this.DefineBaselineVariable(vname_evnt_cnt).then((res) => {
                      if(res)
                        var st = this.DefineDeadBandVariable(vname_deadband).then((res) => {
                          if(res)
                            var st = this.DefineDeadBandHighVariable(vname_deadband_h).then((res) => {
                              if(res)
                                var st = this.DefineDeadBandLowVariable(vname_deadband_l).then((res) => {
                                  if(res)
                                  {
                                    // scope variables
                                    var vars = [ 
