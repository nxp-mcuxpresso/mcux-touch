...
templates["filedef2"] = "<div class=\"touchsw-table-line\" {{#hidden}}style=\"display:none;\"{{/hidden}}>"+
                        "<p class=\"touchsw-table-name\">{{label}} </p>"+
                        "<input class=\"touchsw-table-input\" type=\"text\" name=\"system-period-time\" id=\"{{nodepath}}-{{rndid}}boardfile\" value=\"{{default}}\"></input>"+
                        "<button class=\"touchsw-table-button\" onclick=\"{{onclick}}('{{nodepath}}-{{rndid}}')\">{{btnname}}</button>"+
                        "{{#newwindow}} <button class=\"touchsw-table-button\" onclick=\"openfiledef(this, '{{nodepath}}-{{rndid}}');\" title=\"{{newwindowhint}}\" alt=\"{{newwindowhint}}\">{{newwindowlabel}}</button>{{/newwindow}}"+
                        templates["sticon"]+"</div>";
function getsetfiledef2(id, type, val)//get:type = 0 , set: type= 1, label: type= 2, visibility: type= 3
{
	var obj = document.getElementById(id+"boardfile");
	if(obj){ 
		if(type == 1){ 
			obj.value = val; 
		}
...
function checkboxitemonchange(obj, id)
{
	...
	var descrPathObj = GetDescrObj(path);
	var val = getsetcheckboxitem(rndid, 0, 0); 
	...
	if(!('nt_electrode_module_disabled' in elBaseObj))
		elBaseObj['nt_electrode_module_disabled'] = {};
  var basepath = ReducePath(path, 1);
  var moduleBaseObj = GetDescrObj(basepath);
	...
	if(val)
	{
		if(basepath == "nt_module")
    {
      controlcheckboxitem(rndid, moduleBaseObj['links'], elBaseObj['links'], 0, id2);
		  elBaseObj['nt_electrode_module_disabled'][rndid] = id2;
    }
    else
    {
	...
	else
	{
		if(basepath == "nt_module")
    {
      controlcheckboxitem(rndid, moduleBaseObj['links'], elBaseObj['links'], id2, 0);
		  delete elBaseObj['nt_electrode_module_disabled'][rndid];
    }
    else
    {
...
function EnableAllElCheckboxes(path) 
{
	if(path == "")
		return;
	var controlBaseObj = GetDescrObj("nt_control");
  ...
  var moduleBaseObj = GetDescrObj("nt_module");
	if(!('nt_electrode_module_disabled' in elBaseObj))
		elBaseObj['nt_electrode_module_disabled'] = {};
  ...
  if((elBaseObj) && (controlBaseObj))
  {
    ...
    for(linkindex in elBaseObj['links'])
    {
      ...
      controlcheckboxitem(0, moduleBaseObj['links'], elBaseObj['links'], elid, 0);
    }
  }
}
...
templates["tablestartsub"] = "<div id=\"{{nodepath}}-{{rndid}}div\"><div class=\"touchsw-table\" id=\"{{nodepath}}\">";
...
function GetVarStr(obj, nodeid)
{
	...
	{
		var cnt = 0;
		var moduleObj = GetDescrObj("nt_module");
		var moduleindex = 0;
    var cntEl = [];
    var exitloop = 0;
    for(index in moduleObj["links"])
    {
    cntEl[index] = 0;
    }
		//get linked path
		if(("links" in descrBasePathObj) && ("links" in moduleObj))
		{
			for(index in descrBasePathObj["links"])
			{
				for(elid in descrBasePathObj["links"][index])
				{
			for(index in moduleObj["links"])
			{
					for(id in moduleObj["links"][index])
					{
						checkboxid = id+"_"+elid;

						if(getsetcheckboxitem(checkboxid, 0, 0 &1))
						{
							moduleindex = index
							if(id+"_"+rndid == checkboxid)
							{
								cnt = cntEl[index];
								exitloop = 1;
								break;
		...
		strname = strname.replace("replace1", String(moduleindex));
	...
	else if ((basepath.indexOf("tsi_config_t") == 0) || (basepath.indexOf("cs_config_t") == 0) || (basepath.indexOf("nt_module") == 0) || (basepath.indexOf("nt_tsi_recalib_config") == 0))
	{
		var basepath = ReducePath(path, 8);
		var descrPathObj = GetDescrObj2(path);
		...
		strname = strname.replace("replace1", cnt);
...
function OnClickWrite(obj, nodeid)
{
	...
	SetStatusLineRaw(nodeid, STATUS_CMD_SENDING, "Writing var."+"["+debug_get_timestamp()+"]");
	...
	if((nodeid.indexOf("nt_control_proxi.gesture") >=0) || (nodeid.indexOf("nt_control_keypad.multi_touch") >=0) || (nodeid.indexOf("nt_control_keypad.groups") >=0))
	{
		var i = 0;
		var ptr = 0;
		var st = pcm_read_ptr_new(strname).then((res) => {
			ptr = res.data;
			var data = 0;
			if(ptr)
			{
				if(ptr<0x800000)
				{
					var msg = "Variable "+strname+" is probably read only on address 0x"+ptr+"."+"["+debug_get_timestamp()+"]";
				...
				{
					i=j+Number(1);
					var st = pcm_write_var_new(ptr+Number(0), 4, Number(arr[0+(Number(j))])).then((res2) => {
						var msg = "Write to variable "+strname+"...0x"+ptr.toString(16)+" ("+nodeid+") was successful."+"["+debug_get_timestamp()+"]";
						debug_print(msg, false);
						SetStatusLineRaw(nodeid, STATUS_OK, msg);
					})
					.catch((err) => {
						var msg = "Failed write "+strname + ", check connection. Reported error:"+err+"["+debug_get_timestamp()+"]";
						debug_print(msg, true);
						SetStatusLineRaw(nodeid, STATUS_ERROR, msg);
						this.OnError(err);
					});
			...
			else
			{
				...
				var msg = "Read from variable "+strname+"... ("+nodeid+") was successful."+"["+debug_get_timestamp()+"]";
				debug_print(msg, false);
				SetStatusLineRaw(nodeid, STATUS_OK, msg);
			}
		})
		.catch((err) => {
			...
			var msg = "Symbol "+strname+" not found."+err.msg+"["+debug_get_timestamp()+"]";
			SetStatusLineRaw(nodeid, STATUS_ERROR, msg);
		});
	}
		...
		var st = pcm_read_ptr_new(strname).then((res) => {
			...
			if(ptr)
			{
				...
				for(j=0; j<cnt; j++)
				{
					...
					var prom = [];
					for(var k=0; k<4; k++)
						prom[k] = (ptr+Number(k*8), 8, Number(arr[k+(Number(j)*4)]));
	...
	{
		return pcm_get_symbol_info_new(strname).then((res) => {
			if(res.xtra.addr<0x800000)
			{
				var msg = "Variable "+strname+" is probably read only on address 0x"+res.xtra.addr.toString(16)+"."+" ["+debug_get_timestamp()+"]";
				debug_print(msg, true);
				SetStatusLineRaw(nodeid, STATUS_ERROR, msg);
				return false;
			}
			var val = GetHtmlObjNum(nodeid, out.template, 0);
			var st = pcm_write_var_new(strname, res.xtra.size, val).then((res2) => {
				var msg = "Write to variable "+strname+"...0x"+res.xtra.addr.toString(16)+" ("+nodeid+") was successful."+"["+debug_get_timestamp()+"]";
				debug_print(msg, false);
				SetStatusLineRaw(nodeid, STATUS_OK, msg);
			})
			.catch((err) => {
				var msg = "Failed write "+strname + ", check connection. Reported error:"+err.msg+"["+debug_get_timestamp()+"]";
				debug_print(msg, true);
				SetStatusLineRaw(nodeid, STATUS_ERROR, msg);
				this.OnError(err.msg);
			});
...
{
	SetStatusLineRaw(nodeid, STATUS_CMD_SENDING, "Reading var."+"["+debug_get_timestamp()+"]");
	var out = GetVarStr(obj, nodeid);
	var strname = out.name;
	var data = 0;
		...
		var st = pcm_read_ptr_new(strname).then((res) => {
			...
			if(ptr)
			{
				...
				Promise.all(prom).then((values) => {
					...
					var val = SetHtmlObjNum(nodeid, out.template, data);
					var msg = "Read from variable "+strname+"...0x"+values[0].name.toString(16)+" ("+nodeid+") was successful."+"["+debug_get_timestamp()+"]";
					debug_print(msg, false);
					SetStatusLineRaw(nodeid, STATUS_OK, msg);
				});
			}
	...
	else if(nodeid.indexOf("nt_control_matrix.gesture") >=0)
	{
		...
		var st = pcm_read_ptr_new(strname).then((res) => {
			...
			if(ptr)
			{
				...
				for(i=0; i<20*4; i += 4)
				{
					prom[i] = pcm_read_var_new(ptr+Number(0), 8);
					prom[i+Number(1)] = pcm_read_var_promise(ptr+Number(8), 8);
					prom[i+Number(2)] = pcm_read_var_promise(ptr+Number(16), 8);
					prom[i+Number(3)] = pcm_read_var_promise(ptr+Number(24), 8);
					ptr=ptr+Number(32);
				}
				Promise.all(prom).then((values) => {
					data = "(uint64_t []){";
					for(i=0; i<20; i += 4)
					{
						...
						var f2x=0;
						var f2y=0;
						if("data" in values[i+Number(0)])
							f1x=values[i+Number(0)].data;
						if("data" in values[i+Number(1)])
							f1y=values[i+Number(1)].data;
						if("data" in values[i+Number(2)])
							f2x=values[i+Number(2)].data;
						if("data" in values[i+Number(3)])
							f2y=values[i+Number(3)].data;
	...
	{
		return pcm_get_symbol_info_new(strname).then((res) => {
			var st = pcm_read_var_new(strname, res.xtra.size).then((res2) => {
				data = res2.data;
				var val = SetHtmlObjNum(nodeid, out.template, data);
				var msg = "Read from variable "+strname+"...0x"+res.xtra.addr.toString(16)+" ("+nodeid+") was successful."+"["+debug_get_timestamp()+"]";
				debug_print(msg, false);
				SetStatusLineRaw(nodeid, STATUS_OK, msg);
			})
			.catch((err) => {
				var msg = "Failed read "+strname + ", check connection. Reported error:"+err.msg+"["+debug_get_timestamp()+"]";
				debug_print(msg, true);
				SetStatusLineRaw(nodeid, STATUS_ERROR, msg);
				this.OnError(err.msg);
			});
...
function onchangetableenumsubstrshareditem(obj, id)
{
    ...
    for(linkindex in elbasePathObj['links'])
    {
    	...
    	if(rndid != elid)
    	{
            if((descrPathObj['lasttext']  == 'None') && (selectedElName != 'None'))
            {
              ...
              if("keydetector_interface_electrode_disabled" in elbasePathObj)
              {
              ...
              if ('share_tsi_hw_cfg_disabled' in elbasePathObj)
              {
                if(!(rndid in elbasePathObj["share_tsi_hw_cfg_disabled"]))
                  elbasePathObj["share_tsi_hw_cfg_disabled"].push(rndid);
              }
            }
            else if(selectedElName == 'None')
            {
              ...
              {
                if(rndid in elbasePathObj["share_tsi_hw_cfg_disabled"])
                  elbasePathObj["share_tsi_hw_cfg_disabled"].splice(rndid,1);
              }
            }
            else
            {
    ...
    if("keydetector_interface" in elbasePathObj)
    {
    ...
    if("share_tsi_hw_cfg" in elbasePathObj)
    {
      var hw_type = GetHtmlObjNum(basepath+".interface-"+rndid, elbasePathObj['share_tsi_hw_cfg']['template']);
      var hwtypepath = elbasePathObj['interface']['default'][hw_type]['inclsubstructenum']
      var hwdetObj = GetDescrObj(hwtypepath);
      if(hwdetObj)
      {
        if (!('links' in hwdetObj))
          hwdetObj['links'] = [];
        var idfound = -1;
        for(linkindex in hwdetObj['links'])
        {
          var id = GetLinkId(hwdetObj['links'][linkindex])
          if(id == rndid)
          {
            idfound = linkindex;
            hwdetObj['links'].splice(linkindex,1);
            AddDellHwWebContent(rndid, 9);//register rndid, which will be disabled all the time
            AddDellHwWebContent(rndid, 5);//hide
          }
        }
        if(selectedElName == 'None')
        {
          ...
          {
            link = {};
            link[rndid] = basepath+".interface.tablediv";
            hwdetObj["links"].push(link);
            AddDellHwWebContent(rndid, 10);//unregister rndid, which wil be disabled all the time
            AddDellHwWebContent(rndid, 4);//visible
          }
        }
      }
  }
...
function getsettableenumsubstrshareditem(id, type, val)
{
	...
	{ 
		...
		{
			var descrPathObj = GetDescrObj(GetPathFromPathId(id));
			if("default" in descrPathObj)
			{
				if(descrPathObj["default"][obj.selectedIndex]["intval"])
					return descrPathObj["default"][obj.selectedIndex]["intval"];
...
function onchangetableenumhwinterface(obj, id)/*1 set val, 0: return val*/
{
  ...
  var recalibhtmlobj = document.getElementById("nt_tsi_recalib_config.tablestart-"+rndid+"div");
  if(recalibhtmlobj)
    recalibhtmlobj.style.display = val ? 'None' : 'block';

  UpdateSafetyVisibility(rndid);  
  
  enumonchange(obj, id);
}

function onaddtableenumhwinterface(obj, id, operation)
{
  AddDellHwWebContent(id, operation);
}


templates["tableend"] = "</div>"; 
templates["tableendsub"] = "</div></div>"; 
...
templates["incrementer"] = templates["labelpart1"] + "<div class=\"adjust-number bg-arrow\">"+
										 "<input type=\"text\" id=\"{{nodepath}}-{{rndid}}val\" value=\"{{default}}\" readonly />"+
...
function enumonchange(obj, id)
{
    ...
    if(index >= 0)
    {
    	...
    	if('inclsubstructenum' in datadefobj[index])
    	{
        	...
        	var filter = "";
        	if("imgdescrfilter" in descrPathObj)
        		filter = descrPathObj["imgdescrfilter"];
        	str = GetWebContentFromObj2(subdivdescrPathObj, datadefobj[index]['inclsubstructenum'], "template", subdivdescrPathObj['rndid'], filter);
    	}
...
function sructname_sync(thisobj, id)
{
  ...
  if("hwtypesync" in subdivdescrPathObj)
  {
    var args = [];
    CallFunctStr(subdivdescrPathObj["hwtypesync"], args);
  }
...
function DelSubStructTab(path, id, resetindex)
{
    ...
    if ('inclsubstructdyn' in divDescrPathObj)
    {
        ...
        for(arrindex in arr)
        {
            ...
            //insert tsi/cs configuration
            if ('interface' in subdivDescrPathObj)
            {
              if('onaddcbck' in subdivDescrPathObj['interface'])
              {
                if("hwtypesync" in subdivDescrPathObj['interface'])
                {
                  var args = [];
                  CallFunctStr(subdivDescrPathObj['interface']["hwtypesync"], args);
                }
                var finalArgs = [];
                finalArgs.push(null);
                finalArgs.push(id);
                finalArgs.push(2);//operation del
                var val = CallFunctStr(subdivDescrPathObj['interface']["onaddcbck"], finalArgs);
                //register rndid to tsi/cs struct
                if('default' in subdivDescrPathObj['interface'])
                {
                  for(index in subdivDescrPathObj['interface']['default'])
                  {
                    if('inclsubstructenum' in subdivDescrPathObj['interface']['default'][index])
                    {
                      substructObj = GetDescrObj(subdivDescrPathObj['interface']['default'][index]['inclsubstructenum']);
                      if("links" in substructObj)
                      {
                        for ( subitem in substructObj['links'] )
                        {
                          if(id in substructObj['links'][subitem])
                          {
                            substructObj['links'].splice(subitem, 1);
                          }
                        }
                      }
                    }
                ...
                //unregister id from substruct
                if ('inclsubstructdyn' in divDescrPathObj)
                {
                  var modulebase = divDescrPathObj['inclsubstructdyn']
                  var linkStructDescrPathObj = GetDescrObj2(modulebase+".safety_params.gpio.inclsubstruct");
                  if(linkStructDescrPathObj && ("inclsubstruct" in linkStructDescrPathObj))
                  {
                    var linkObj = GetDescrObj2(linkStructDescrPathObj["inclsubstruct"]);
                    if("links" in linkObj)
                    {
                      for ( subitem in linkObj['links'] )
                      {
                        if(id in linkObj['links'][subitem])
                        {
                          linkObj['links'].splice(subitem, 1);
                        }
                      }
                    }
                  }
                  linkStructDescrPathObj = GetDescrObj2(modulebase+".recalib_config.inclsubstruct");
                  if(linkStructDescrPathObj && ("inclsubstruct" in linkStructDescrPathObj))
                  {
                    var linkObj = GetDescrObj2(linkStructDescrPathObj["inclsubstruct"]);
                    if("links" in linkObj)
        ...
        for(key in inclstructDescrPathObj['syncs'])
        {
        	...
        	else if (('activetmpl' in syncdescrPathObj) && (syncdescrPathObj['template'] == 'tableeditshilding'))
        	{
        		...
        		//unregister disabled electrode of module from nt_electrode array
        		if('nt_electrode_module_disabled' in syncdescrPathBaseObj)
        		{
        			for( elid in syncdescrPathBaseObj['nt_electrode_module_disabled'])
        			{
        				if(syncdescrPathBaseObj['nt_electrode_module_disabled'][elid] == id)
        				{
        					delete syncdescrPathBaseObj['nt_electrode_module_disabled'][elid];
        				}
        			}
        		}
        	}
      ...
      if(!CheckLabel(path, label))
      {
	    ...
	    if('inclsubstructdyn' in divDescrPathObj)
	    {
            ...
            else if(typeof(subdivDescrPathObj["syncs"]) === 'string')
            {
            	...
            	if("syncs4" in subdivDescrPathObj)
            	{
                	var s4 = subdivDescrPathObj["syncs4"];
                	subdivDescrPathObj["syncs"].push(s4);
            	}
            }
            var to_push = true;
            		...
            		for(linkbase in electDescrPathObj['links'])
            		{
                			...
                			if(path == "nt_controls")
                      {
                        for(disabledid in electDescrPathObj["nt_control_disabled"])
                  			...
                  			for(disabledid in electDescrPathObj["nt_electrode_disabled"])
                  			{
                  				if(electDescrPathObj["nt_electrode_disabled"][disabledid] == linkid)
                  				{
                  					VisibilityHtmlObjVal(syncDescrPathObj["rndid"] + "_"+linkid, syncDescrPathObj['activetmpl'], 1);
                  					break;
                  				}
                      ...
                      if(path == "nt_modules")
                      {
                  			for(disabledid in electDescrPathObj["nt_electrode_module_disabled"])
                  			{
                  				if(electDescrPathObj["nt_electrode_module_disabled"][disabledid] == linkid)
                  				{
                  					VisibilityHtmlObjVal(syncDescrPathObj["rndid"] + "_"+linkid, syncDescrPathObj['activetmpl'], 1);
                  					break;
                  				}
            ...
            //load all modules to module hw config selector
            if ('share_tsi_hw_cfg' in subdivDescrPathObj)
            {
            	if(!("share_tsi_hw_cfg_disabled" in subdivDescrPathObj))
            		subdivDescrPathObj["share_tsi_hw_cfg_disabled"] = [];
            	for(linkbase in subdivDescrPathObj['links'])
        		{
        			for(linkid in subdivDescrPathObj['links'][linkbase])
        			{
            			if(rndid != linkid)
            			{
            				var syncDescrPathObj = subdivDescrPathObj;
                			var val = GetHtmlObjVal(subdivDescrPathObj['sructname']['nodepath'] + "-" + linkid, subdivDescrPathObj['sructname']['template']);
                			AddHtmlObjVal(divDescrPathObj['inclsubstructdyn'] +".share_tsi_hw_cfg-"+rndid , rndid + "_"+linkid, subdivDescrPathObj['share_tsi_hw_cfg']['template'], val);
                            for (disabledid in syncDescrPathObj["share_tsi_hw_cfg_disabled"]) {
                                if ((syncDescrPathObj["share_tsi_hw_cfg_disabled"][disabledid] == linkid)&&(rndid != linkid)) {
                                    VisibilityHtmlObjVal(rndid + "_" + linkid, subdivDescrPathObj['share_tsi_hw_cfg']['activetmpl'], 1);
                                    break;
                                }
                            }
            			}
            ...
            if ('interface' in subdivDescrPathObj)
            {
              if('onaddcbck' in subdivDescrPathObj['interface'])
              {
                ...
                var finalArgs = [];
                finalArgs.push(null);
                finalArgs.push(rndid);
                finalArgs.push(1);//operation add
                var val = CallFunctStr(subdivDescrPathObj['interface']["onaddcbck"], finalArgs);
                //register rndid to tsi/cs struct
                if('default' in subdivDescrPathObj['interface'])
                {
                  for(index in subdivDescrPathObj['interface']['default'])
                  {
                    if('inclsubstructenum' in subdivDescrPathObj['interface']['default'][index])
                    {
                      ...
                      var link = {};
                      link[rndid] = path+".interface";
                      substructObj["links"].push(link);
                    }
                  }
                }
              }
            ...
            if(path == "nt_modules")
            {
                val = SetHtmlObjVal(subdivDescrPathObj['instance']['nodepath'] + "-" + rndid, subdivDescrPathObj['instance']['template'], instance_index-1);
            }
	    }
	}
	else
...
{
  if(document.NTElectrode.All[Number(index)])
    document.NTElectrode.All[Number(index)].OnLinkClicked()
}
...
templates["tableenummodsel"] = "<div class=\"touchsw-table-line\" {{#hidden}}style=\"display:none;\"{{/hidden}}  id=\"{{nodepath}}-{{rndid}}tr\">"+
									 "<p class=\"touchsw-table-name\" {{#descr}}title=\"{{descr}}\" {{/descr}}>{{label}}</p>"+
									 "<div class=\"select-wrap\">"+
										"<select class=\"choose unselected\" name=\"choose_user\" id=\"{{nodepath}}-{{rndid}}\" onchange=\"onchangetableenummodsel(this, '{{nodepath}}-{{rndid}}')\">"+
											"{{#default}}<option class=\"selection-name\" {{#descr}}title=\"{{descr}}\" {{/descr}} value=\"{{item}}\"  {{selected}}>{{sublabel}}</option>{{/default}}"+
										"</select>"+
									"</div>"+
							"</div>";
...
function onchangetableenummodsel(obj, id)/*1 set val, 0: return val*/
{
  if(!obj)
    return;
  var objsel = obj;
  ...
  for (i = 0; i<objsel.length; i++)
  {
    var show = 0;
    var val =  0;
    var rndid = objsel[i].value;
    if(i == objsel.selectedIndex)
    {
      val = getsettableenum(path+"-"+rndid, 7, 0); //read the index
      show = 1;
    }
    AddDellHwWebContent(rndid, show?4:5);
    var path = "nt_module.interface";
    
    var rndid = GetIdFromPathId(rndid);
