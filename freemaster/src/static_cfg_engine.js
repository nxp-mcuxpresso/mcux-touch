...
{
	return IsValidDataPath(document.data, idpath);
}

function GetDescrObj(idpath)
{
    return GetDataObj(document.data, idpath);
}

function GetDescrObj2(idpath)
{
    return GetDataObj2(document.data, idpath);
}
...
function GetDataFromWeb2(obj, nodepath, key, rndid, nolinks)
{
    ...
    if(typeof(obj) != 'object')
        return "";
    if(obj['cfgfile'])
        debug_log("cfgfile:" + obj['cfgfile'] + " :"+rndid);
                  ...
                  if(!syncObjDescrPathObj)
                    continue;
            			...
            			for(control in controls)
            			{
            				...
            				debug_log(controls[control]['key'] + " " + rndid + ":"+cbval);
            				var cbarr = {};
...
function DeleteDynSubStruct(obj, path, resetindex)
{
    if("inclsubstructdyn" in obj)
    {
        ...
        var incrobj = GetDescrObj(obj['inclsubstructdyn']+".sructname");
        if(incrobj && ('increment' in incrobj))
          incrobj['increment'] = 0;
    	}
    }
}
...
function SetDataToWeb2(obj, cfgdata, key, id, rndid, nolinkarg)
{
	...
	if(obj['cfgfile'])
        debug_log("cfgfile:" + obj['cfgfile'] + " :"+rndid + "  :" + id);
    if(!rndid)
    	rndid = 1;
  ...
  var rndidorig = rndid;
    ...
    for( link in linksobj)
    {
        ...
        if('sructname' in obj)
        {
            ...
            if('nostruct' in subobj)
                structstring = "";
        		structnamestr = conststr + structstring +" "+ nodepath + " " +subobj['linkedname']+"_"+elname+ " = { \n";
                structendstr = "};\n";        		
        	}
        ...
        else if (('outfile' in obj)&& (base_key != 'sructname'))
        {
            ...
            {
            	var tsi_type = false;
              if((obj['inclsubstruct'] == "nt_tsi_recalib_config") || (obj['inclsubstruct'] == "nt_module_safety_gpio_params") || (obj['inclsubstruct'] == "tsi_config_t") || (obj['inclsubstruct'] == "cs_config_t"))
              {
                var base_path = GetDescrObj("nt_module");
                if(GetValFromPage(rndidorig, "nt_module.interface") == "&nt_module_tsi_interface")
                  tsi_type = true;
              }
            	var base_path = ReducePath(nodepath, 1);
              if(obj['inclsubstruct'] == "nt_module_safety_gpio_params")//exception for nt_module_safety_gpio_params .. it has 2 dots in nodepath
                base_path = ReducePath(base_path, 1);
              var baseDescrPathObj =  GetDescrObj(base_path+".sructname");
              var name2 = GetHtmlObjVal(base_path+".sructname-"+rndidorig, baseDescrPathObj['outfile']);
              
              if((obj['inclsubstruct'] == "cs_config_t") || (obj['inclsubstruct'] == "tsi_config_t"))
              {
                var sel_type = GetValFromPage(rndidorig, "nt_module.interface")
                var typeObj =  GetDescrObj("nt_module.interface.default");
                // get the name of structure
                var structName = "";
                for(type in typeObj)
                {
                  if("item" in typeObj[type])
                  { 
                    if(typeObj[type]["item"] == sel_type)
                    {
                      if("inclsubstructenum" in typeObj[type])
                        structName = typeObj[type]["inclsubstructenum"];
                    }
                  }
                }
                //detect shared cfg
                {
                  var moduleDescrPathObj = GetDescrObj(base_path);
                  var sharedkeydetcfg = GetHtmlObjVal(base_path+".share_tsi_hw_cfg-"+GetIdFromPathId(nodepath), moduleDescrPathObj["share_tsi_hw_cfg"]["template"]);
                  var sharecfg = "None";
                  if(sharedkeydetcfg)
                    sharecfg = sharedkeydetcfg.replace("&", "");
                  if(sharecfg != "None")
                    name2 = sharecfg;
                }
                var name =  structName + "_" +name2;
        	       finalstr = finalstr + "  ." + base_key + " = (void*)&" + name + ",\n";
              }else if(tsi_type)
            	{
                  var path = obj['inclsubstruct']+ ".sructname.linkedname";//used by hw config
                  var name =  GetDescrObj(path) + "_" +name2;
        	    	finalstr = finalstr + "  ." + base_key + " = (void*)&" + name + ",\n";
            	}
            	continue;
            }
...
function GenHFileContent(obj, nodepath, base_key, rndid)
{
    ...
    if(typeof(obj) != 'object')
        return "";
	if(obj['outhfile'])
        debug_log("outhfile:" + obj['outhfile']);
    if(!rndid)
    	rndid = 1;
